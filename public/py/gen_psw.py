...
class PasswordGenerator:
	...
	def ask_yes_no(self, question, default=False):
		if default:
			question += f" [default: {'yes' if default else 'no'}]"
		while True:
	...
	def generate_password(self, length=None):
		...
		# Se la lunghezza non è stata specificata, usa il default
		if length is None:
...
if __name__ == "__main__":
	while True:
		...
		password_length = int(input(f"Quale lunghezza deve avere la password? [default: {generator.default_length}] ").strip() or generator.default_length)
